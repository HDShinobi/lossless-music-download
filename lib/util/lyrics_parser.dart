...
class LyricsParser {
  ...
  static final RegExp _lineTimeTag =
      RegExp(r'\[(\d{1,3}):(\d{1,2})(?:[.:](\d{1,3}))?\]');
  static final RegExp _wordTimeTag =
      RegExp(r'<(\d{1,3}):(\d{1,2})(?:[.:](\d{1,3}))?>');
  ...
  static ParsedLyrics? _parseTtml(String text) {
    try {
      ...
      for (final p in paragraphs) {
        ...
        plain.add(lineText);
        if (begin != null) {
          lines.add(LyricLine(time: begin, end: end, text: lineText, words: words));
        }
      }
      if (lines.isEmpty) {
        return ParsedLyrics(
            synced: false, wordSynced: false, lines: const [], plainText: plain.join('\n'));
      }
